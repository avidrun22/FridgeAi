...
- [ ] **Set up Impact affiliate accounts for Instacart + Walmart** — earnings on Instacart and Walmart reorder clicks are currently $0 because their tracking IDs are placeholders. Target was dropped from the retailer set 2026-04-28 (no payout on grocery). Sign up at [impact.com](https://impact.com) as an affiliate partner (free, ~3-7 days approval). Apply to **Instacart** and **Walmart** programs from the Impact marketplace — both run through Impact, so one dashboard tracks both. Once approved, replace the placeholder `utm_source=ok2eat&utm_medium=affiliate` query params in `App.js` `RETAILERS` array (search "RETAILERS" — three rows). Amazon stays separate via Amazon Associates (already live, tag `ok2eat-20`); payouts there are minimal but the longtail pantry-staple traffic still earns. New retailer order in the picker: Instacart 1st, Amazon 2nd, Walmart 3rd.
