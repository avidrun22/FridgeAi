...
- [ ] **Twice-weekly blog cadence (Tue + Thu)** — the strategic move after the Impact rejection: build content on ok2eat.com so the site has substance to crawl when reapplying in 4–8 weeks. **Scheduled**: scheduled task `ok2eat-blog-post-nudge` (cron `0 9 * * 2,4`) sends a Telegram nudge every Tue/Thu 9am Pacific. **Built 2026-04-29**: `/blog/index.html`, `/blog/styles.css`, `/blog/newsletter.js`, first post `/blog/why-i-built-ok2eat.html` (founder origin story). Nav updated on `ok2eat.html` to include Blog. Email signup form on homepage and at the end of every post. **Founder-series content arc** (4 posts): (1) Why I built ok2eat — origin story [DRAFT WRITTEN], (2) How I cut my grocery bill by optimizing what I already have, (3) How I always know what to buy when I'm not at home, (4) What real spending data taught me about my habits. After the founder series, alternate between food-waste tips, expiry myths, and simple-recipe posts. **Outstanding before launch**: deploy `subscribe-newsletter` Edge Function (`supabase functions deploy subscribe-newsletter`), create the Resend audience and set `RESEND_AUDIENCE_ID` secret, run `/deploy` to push the new blog pages to Netlify.
