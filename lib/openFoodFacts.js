...
async function searchByText(query, { pageSize = 10, timeoutMs = 6000 } = {}) {
  ...
  try {
    // v1.21 — fetch 25 candidates (2.5× pageSize) so the relevance scorer
    // has enough variants to find the generic version. With pageSize=10 and
    // OFF's popularity sort, all 10 returned rows were often branded
    // variants of the same product, with the plain generic version missing
    // entirely. Scoring a bigger pool lets us surface the generic first.
    const fetchSize = Math.max(25, pageSize * 2);
    const params = new URLSearchParams({
      ...
      // arbitrary order. (Our own scoring re-orders before returning.)
    ...
    // yogurt above actual milk). Iteration 2 of the scorer (Greg request,
    // 2026-05-16): generic products should win over branded variants
    // unless the query explicitly names a brand. "whole milk" → plain
    // "Whole Milk", not "Horizon Organic Whole Milk Half Gallon". Then
    // if the user types "horizon whole milk", the brand token bumps that
    // variant ahead.
    //
    // Signal hierarchy:
    //   1. Exact phrase in name           +100  (query as a contiguous
    //                                            substring of name)
    //   2. All tokens in name              +50  (every search token present
    //                                            in name, may be non-contiguous)
    //   3. Token in name                   +10  (per token, additive)
    //   4. Token only in brand              +5  (per token, additive — bumped
    //                                            from +2 so brand-named queries
    //                                            like "horizon whole milk"
    //                                            actually surface Horizon first)
    //   5. Generic-product bonus           +60  (name length ≤ phrase length+10
    //                                            AND no brand listed — basically
    //                                            "the product IS just the thing
    //                                            you searched for")
    //   6. Branded-variant tax            -15  (name length > phrase length×3
    //                                            i.e. lots of extra brand-and-
    //                                            packaging noise after the
    //                                            search term)
    //   7. Score < 10                      → drop the result (single-token
    //                                          brand-only noise)
    // Sort by score desc, then by name length asc (shorter = more generic
    // tiebreak), then by OFF's unique_scans_n popularity order.
    ...
    const scored = normalized.map((p, idx) => {
      ...
      // Per-token signal — name >> brand. Brand bumped to +5 (from +2)
      // so explicit brand queries surface branded variants ahead of
      // generic ones.
      let nameTokensHit = 0;
      for (const tok of tokens) {
        if (name.includes(tok)) { score += 10; nameTokensHit += 1; }
        else if (brand.includes(tok)) { score += 5; }
      }
      // All-tokens-in-name bonus.
      if (nameTokensHit === tokens.length) score += 50;
      // GENERIC-PRODUCT BONUS — the headline change. Reward results
      // where the name is essentially JUST the search term (with at
      // most ~10 chars of trailing context) AND no brand is listed.
      // "Whole Milk" (10 chars, no brand) for query "whole milk" (10
      // chars) → name-len ≤ 20 + no brand → +60.
      const isGeneric = nameLen > 0 && nameLen <= phraseLen + 10 && !brand;
      if (isGeneric) score += 60;
      // BRANDED-VARIANT TAX — penalize results whose name is much
      // longer than the query (typical of branded SKUs that pile on
      // packaging and marketing copy). "Horizon Organic Whole Milk
      // Half Gallon Refrigerated Carton" (~60 chars) for query "whole
      // milk" (10 chars) → 60 > 30 → -15.
      if (phraseLen > 0 && nameLen > phraseLen * 3) score -= 15;
      return { p, score, nameLen, originalIdx: idx };
    });

    // v1.21 fix (Greg "whole milk" 0-results regression, 2026-05-16):
    // Lowered threshold 10 → 5 so brand-only matches pass through, AND
    // added a fallback: when the strict-filtered set is empty but OFF
    // returned non-zero candidates, return the top 5 unfiltered results
    // sorted by score. Better to show "Cabot whole milk cheddar" (weak
    // match) than nothing at all when the user typed "whole milk".
    const sorted = scored.sort((a, b) =>
      (b.score - a.score) ||
      (a.nameLen - b.nameLen) ||
      (a.originalIdx - b.originalIdx)
    );
    const strict = sorted.filter(s => s.score >= 5);
    const ranked = strict.length > 0
      ? strict
      : sorted.slice(0, 5);
    return ranked.slice(0, pageSize).map(s => s.p);
  } catch (e) {
    return [];
  } finally {
    clearTimeout(t);
