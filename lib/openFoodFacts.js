...
async function searchByText(query, { pageSize = 10, timeoutMs = 6000 } = {}) {
  ...
  try {
    ...
    // Relevance guard: split query into tokens (length > 2 to skip stop-
    // words like "to", "of") and require each returned product's name or
    // brand to contain AT LEAST ONE token. Drops the random-product
    // results we saw when OFF's search misfires.
